...
func WriteDocs(tokens []parser.Token, outDir string, sourceFiles []string) error {
	...
	// Copy source token CSS files to output (dogfooding - site uses its own tokens)
	tokensPath := filepath.Join(outDir, "tokens.css")
	if err := concatenateSourceFiles(sourceFiles, tokensPath); err != nil {
		return err
	}
...
// concatenateSourceFiles combines all source CSS files into one output file
func concatenateSourceFiles(sourceFiles []string, outPath string) error {
	...
	f.WriteString("/* Design Tokens - Generated by cssmark */\n\n")
