...
func TestWriteFlatCSSWritesComposableCssmarkSource(t *testing.T) {
	...
	err := WriteFlatCSS([]Token{{
		ID: "action.type.body", Name: "--hb-action-type-body", Type: "typography", Value: map[string]any{
	...
	for path, expected := range map[string]string{
		"index.css":      `@import "./typography.css";`,
		"typography.css": `@property --hb-action-type-body-font-family {`,
	} {
	...
	if !strings.Contains(string(body), `mode-dense: var(--hb-space-3-5);`) {
